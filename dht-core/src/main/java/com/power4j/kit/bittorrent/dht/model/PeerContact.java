...
public class PeerContact {
	...
	public static PeerContact of(Inet4Address address, int port) {
		return new PeerContact(address, port);
	}

	PeerContact(Inet4Address address, int port) {
		this.address = address;
		this.port = port;
	}

	public BepStr compact() {
		return DhtUtil.compactPeerContract(address, port);
	}
